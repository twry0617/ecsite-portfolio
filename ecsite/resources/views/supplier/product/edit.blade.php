...
@section('content')
    ...
    <div class="container mt-5">
        <div class="row bg-light d-flex">
            <form action="{{ route('supplier.product.edit', ['product' => $product->id]) }}" method="post" class="form-inline" enctype="multipart/form-data">
                @csrf
                <div class="">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="price" value="{{ old('price', $product->price) }}">円
                    </div>
                    <div class="form-group">
                        <label class="col-md-6 mx-auto" for="stock">在庫</label>
                        <input class="form-control" type="text" name="stock" value="{{ old('stock', $product->stock) }}">
                    </div>
                    <div class="form-group">
                        ...
                        <label for="size" class="col col-md-5">サイズ</label>
                            ...
                            <select class="form-control" name="size" id="size">
                                @foreach(\App\Enums\OptionSize::values() as $size)
                                    <option value="{{ $size }}">{{ old('size', $size) }}</option>
                                @endforeach
                            </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" name="color" id="color">
                            @foreach(\App\Enums\OptionColor::values() as $color)
                                <option value="{{ $color }}">{{ old('color', $color) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
